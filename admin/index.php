                                ...
                                <table id="datatablesSimple">
                                    ...
                                    <tbody>
    									...
    									while ($row = $stmt->fetch()) {
            								...
            								echo "<td>" . $row['role'] . "</td>";
            								echo "<td><select class='form-select' onchange='updateUser(this);' id=".$row['id']." aria-label='Rol Listesi'>
  													<option disabled selected value='default'>Choose a role...</option>
  													<option value='active'>Active</option>
											...
											echo "<td><button type='button' onclick='deleteUser(".$row['id'].")' class='btn btn-danger'>Delete</button></td>";
											echo "<tr>";
										}
                                    	?>
                                    </tbody>
        ...
        <script type="text/javascript">
           /* $(document).ready(function(){
                $.ajax({
                    ...
                    $('#datatablesSimple2 > tbody > tr').remove();
                    for (var i = 0; i < result.length; i++) {
                        var str = "<tr>"+
                                     "<td>" +result[i].name + "</td>" + 
                                     "<td>" +result[i].status + "</td>"+
                                     "<td><select id='"+result[i].id+"' class='form-select' onchange='updateMachine(this);' aria-label='Rol Listesi'>"+
                                     "<option disabled selected value='default'>Choose a Status...</option>"+
                                                    "<option value='active'>Active</option>"+
                                                    "<option value='passive'>Passive</option>"+
                                                    "<option value='out of order'>Out of Order</option>"+
                                     "</select></td>"+
                                     "<td><button type='button' onclick='deleteMachine("+result[i].id+")' class='btn btn-danger'>Delete</button></td>"
                                    + "</tr>"
                        
                        $("#datatablesSimple2 > tbody").append(str)
                    }
                }});
            });*/
            ...
            function updateUser(select)
            {
                request = $.ajax({
                url: "../api/user-update-api.php",
                type: "put",
                data: JSON.stringify({'id':select.id, 'role':select.value}),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
